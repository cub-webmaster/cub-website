...
<div bind:clientHeight={height} class="absolute bottom-0 w-full">
	...
	<div class="bg-info sm:px-8 px-4 py-4 flex justify-between gap-8 items-center">
		...
		<div class="flex gap-8">
			<!-- I only realised after using it quite a lot that Lucide doesn't have brand labels so here are the raw svgs -->
			<SocialIcon href={links.instagram.url} label={links.instagram.label}>
				{#snippet icon()}
			...
			<SocialIcon href={links.facebook.url} label={links.facebook.label}>
				{#snippet icon()}
					<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
			...
			<SocialIcon href={links.twitter.url} label={links.twitter.label}>
				{#snippet icon()}
					<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
						><title>X</title><path
							fill="currentColor"
